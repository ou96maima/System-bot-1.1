const { ActionRowBuilder, ButtonBuilder, ButtonStyle, ComponentType, ActionRow } = require('discord.js');

module.exports = (client) =>{
    client.pagination = async (interaction, pages, time = 30 * 1000) => {
        try {
            ...
            collector.on('collect', async i => {
                if(i.user.id !== interaction.user.id) return await i.reply({ content: `Only **${interaction.user.username}** can use these buttons!` });
                await i.deferUpdate();
