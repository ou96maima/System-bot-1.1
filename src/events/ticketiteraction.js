  ...
  async execute(interaction, client) {
    if (interaction.customId == "ticketCreateSelect") {
      ...
      const reason = new TextInputBuilder()
        .setCustomId("closeReason Ticket")
        .setRequired(true)
      ...
      const reason = interaction.fields.getTextInputValue("closeReason Ticket");
      await interaction.reply({ content: ` closing this tiket...` });
      setTimeout(async () => {
    ...
    } else if (interaction.customId == "tiketTranscript") {
      ...
      var msg = await interaction.channel.send({
        content: ` Your transcript chache`,
        files: [file],
      });
      var message = ` **Here is your [tiket transcript](https://mahto.id/chat-exporter?url=${
        msg.attachements.first()?.url
